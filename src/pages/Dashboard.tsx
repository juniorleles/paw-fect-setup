  ...
  return (
    <div className="flex-1 p-4 md:p-8 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Config summary + WhatsApp */}
        <div className="space-y-6">
          {/* WhatsApp Status Card */}
          <Card className={`border-none shadow-md overflow-hidden ${
            whatsappStatus === "connected" ? "bg-success/5" : whatsappStatus === "pending" ? "bg-accent/5" : "bg-destructive/5"
          }`}>
            <CardContent className="py-4 px-5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                    whatsappStatus === "connected" ? "bg-success/15" : whatsappStatus === "pending" ? "bg-accent/15" : "bg-destructive/15"
                  }`}>
                    <Zap className={`w-5 h-5 ${
                      whatsappStatus === "connected" ? "text-success" : whatsappStatus === "pending" ? "text-accent" : "text-destructive"
                    }`} />
                  </div>
                  <div>
                    <p className="text-sm font-bold">
                      {whatsappStatus === "connected" ? "WhatsApp Conectado" : whatsappStatus === "pending" ? "Aguardando Conexão" : "WhatsApp Desconectado"}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {whatsappStatus === "connected" ? "Sua secretária está ativa" : "Reconecte para ativar"}
                    </p>
                  </div>
                </div>
                <span className={`w-3 h-3 rounded-full flex-shrink-0 ${
                  whatsappStatus === "connected" ? "bg-success" : whatsappStatus === "pending" ? "bg-accent animate-pulse" : "bg-destructive"
                }`} />
              </div>
